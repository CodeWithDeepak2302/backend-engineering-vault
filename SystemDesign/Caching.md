...
* **The Role of Cache-Control Headers:** CDNs respect standard HTTP headers. If your origin server sends a response with `Cache-Control: public, max-age=60`, the CDN will store that JSON response for 60 seconds. Any subsequent request for that same endpoint within that minute is served directly from the CDN's Point of Presence (PoP), never touching your backend.
* **GraphQL Caching:** Since GraphQL typically uses POST requests (which aren't cached by default), modern CDNs and tools like Apollo or Fastly use Persisted Queries. This turns a complex POST request into a cacheable GET request tied to a unique hash.
* **Edge Computing (Edge Functions):** Services like Cloudflare Workers or AWS Lambda@Edge allow you to run backend logic directly at the CDN level. You can:
...
### The Flow: Client → CDN → Origin
In a standard setup, your DNS records point to the CDN's IP addresses, not your server's. Therefore, the CDN acts as a "Reverse Proxy."

**The Step-by-Step Flow:**
1. **The Request:** The user enters `api.example.com`. The DNS resolves to the nearest CDN Point of Presence (PoP).
2. **The Trigger:** The CDN receives the request. Instead of just looking for a file in its cache, it triggers an Edge Function (like a Cloudflare Worker).
3. **The Logic:** The function executes code. It can:
   * Inspect Headers: Check if a JWT token is valid (Authentication).
   * Modify the Request: Rewrite the URL or add custom headers.
   * Check the Cache: Look for a stored API response.
4. **The Decision (Branching):**
   * **Path A (Edge-Only):** If the function has the answer (e.g., an auth failure or a cached response), it returns it to the user immediately. The request never touches your server.
   * **Path B (Origin Fetch):** If the function needs fresh data, it calls your origin server, gets the response, potentially modifies it (e.g., stripping sensitive fields), and then sends it to the user.

### Real-World Interview Example: A/B Testing
If an interviewer asks, "How do we show Version A of a site to 50% of users and Version B to the other 50% without a flicker or slow load times?"
* **The Wrong Way:** Let the client-side JavaScript do it (causes a "flash" of original content).
* **The Standard Way:** Do it at the origin server (adds latency for the round trip).
* **The Edge Way:** Use an Edge Function. The CDN sees the request, looks at a cookie, and "stitches" together the correct version of the page or redirects to the correct bucket before the user's browser even starts rendering.
...
![[In_process_caching.png|811]]

**Trade Off:** In-process caching is blazing fast, but it comes with obvious limitations. Each instance of your application has its own cache, so cached data is not shared across servers. If one instance updates or invalidates a cached value, the others will not know.
...
### Read-Through Caching -- Used in Apprise
...
The pattern is simple. Identify the performance problem, quantify it with rough numbers, and explain how caching solves it. You can use our Numbers to Know get a sense of reasonable database and cache latencies.
