...
2. **GraphQL Caching**
   Since GraphQL typically uses `POST` requests (which aren't cached by default), modern CDNs and tools like Apollo or Fastly use **Persisted Queries**. This turns a complex `POST` request into a cache able `GET` request tied to a unique hash.
...
If an interviewer asks, _"How do we show Version A of a site to 50% of users and Version B to the other 50% without a flicker or slow load times?"_
